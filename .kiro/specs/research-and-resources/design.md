...
switch (agentSlug) {
  case "professor":
    return res.json({
      ok: true,
      research: null,
      message: "Professor agent does not use external research.",
    });
  case "recruiter":
  case "investor":
  ...
  "resourceBriefs": [
    {
      ...
      "improvement": "Quantify impact in the Result step",
...
const scrapeTool = tool(
  ...
  },
  {
    name: "scrapeWebsite",
    description:
      "Fetch a single URL and return its main-content markdown. Truncated to 6000 chars.",
...
_Company research (recruiter / investor):_
...
_Resource curation (one call per brief):_
