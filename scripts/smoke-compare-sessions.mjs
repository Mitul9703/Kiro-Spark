...
const body = {
  ...
  baselineSession: fixtureSession({
    id: "sess-baseline",
    score: 64,
    endedAtOffsetMs: 3 * 24 * 3600 * 1000,
  }),
};
...
if (!cmp || typeof cmp !== "object") {
  console.error("missing comparison", json);
  process.exit(1);
}

const allowedTrend = new Set(["improved", "mixed", "similar", "declined"]);
if (!allowedTrend.has(cmp.trend)) {
  console.error("invalid trend", cmp.trend);
  process.exit(1);
}
...
for (const m of cmp.metrics) {
  if (typeof m.delta !== "number") {
    console.error("metric.delta not number", m);
    process.exit(1);
  }
  if (!["improved", "declined", "similar"].includes(m.trend)) {
    console.error("metric.trend invalid", m);
    process.exit(1);
  }
}
console.log(
  "OK trend=%s metrics=%d %s",
  cmp.trend,
  cmp.metrics.length,
