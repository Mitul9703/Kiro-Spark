...
const body = {
  ...
  transcript: [
    ...
    {
      role: "User",
      text: "I led a team of three on a payments redesign that cut p99 latency by 40%.",
    },
  ],
  durationLabel: "04:12",
...
if (typeof json.evaluation?.score !== "number") {
  console.error("missing evaluation.score", json);
  process.exit(1);
}
if (!Array.isArray(json.evaluation?.metrics) || json.evaluation.metrics.length < 1) {
  console.error("metrics missing", json);
  process.exit(1);
}
