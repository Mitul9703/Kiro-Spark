...
if (fs.existsSync(fixturePath)) {
  ...
  if (!json.ok) { console.error('not ok', json); process.exit(1); }
  if (typeof json.contextText !== 'string') { console.error('missing contextText', json); process.exit(1); }
  if (typeof json.contextPreview !== 'string') { console.error('missing contextPreview', json); process.exit(1); }
  console.log('OK fileName=%s contextText.length=%d',
    json.fileName ?? '(unset)',
  ...
  console.log('OK no-fixture mode: route mounted, 400 on missing file (%s)', json.error ?? '(no error key)');
